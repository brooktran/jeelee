...
public class BruteForceCoding {
  ...
  //Numbers of bytes in Java integer primitive:
  ...
  /**
	 * Byte array to decimal string. This method prints each byte from the given
	 * array as an unsigned decimal value. BYTEMASK keeps the byte value from
	 * being sign-extended when it is converted to an int in the call to
	 * append(), thus rendering it as an unsigned integer.
	 * 
	 * @param bArray
	 *            the b array
	 * 
  ...
  public static int encodeIntBigEndian(byte[] dst, long val, int offset, int size) {
    for (int i = 0; i < size; i++) {
      dst[offset++] = (byte) (val >> ((size - i - 1) * Byte.SIZE));//右移
    }
    return offset;
  }
  ...
  public static long decodeIntBigEndian(byte[] val, int offset, int size) {
    long rtn = 0;
    for (int i = 0; i < size; i++) {
      rtn = (rtn << Byte.SIZE) | ((long) val[offset + i] & BYTEMASK);//左移  或操作
    }
    return rtn;
  }
